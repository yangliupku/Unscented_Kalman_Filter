...
void UKF::ProcessMeasurement(MeasurementPackage meas_package)
{
    if (!is_initialized_)
    {
        ...
        if (meas_package.sensor_type_ == MeasurementPackage::RADAR)
        {
            ...
            double k = tan(meas_package.raw_measurements_[1]);
            double rho = meas_package.raw_measurements_[0];

            x_(0) = copysign(k, rho / sqrt(1 + k * k));
            x_(1) = x_(0) * k;
...
{
    // generate augmented sigma points, and predict sigma points at k+1 using state equation
    ...
    // predict sigma points using state equation
...
void UKF::UpdateLidar(MeasurementPackage meas_package)
{
    ...
    //create matrix for sigma points in measurement space Z k|k+1
    ...
    // calculate Kalman gain, update state and covariance
